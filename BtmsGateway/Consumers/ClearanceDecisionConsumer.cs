    ...
    ILogger logger) : IConsumer<IConsumerContext<ResourceEvent<CustomsDeclaration>>>
{
    public async Task OnHandle(IConsumerContext<ResourceEvent<CustomsDeclaration>> context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                logger.Error("{MRN} Failed to send clearance decision to Decision Comparer. Decision Comparer Response Status Code: {StatusCode}, Reason: {Reason}, Content: {Content}",
                    mrn,
                    result.StatusCode,
                    result.ErrorMessage,
                    result.ResponseContent);
                throw new ClearanceDecisionProcessingException($"{mrn} Failed to send clearance decision to Decision Comparer.");
            }
