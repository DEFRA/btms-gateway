...
public class JsonToSoapConverterTests
{
    ...
    [InlineData("AlvsToCdsDecisionNotification.xml", "DecisionNotification", SoapType.AlvsToCds, "DecisionNotification.json")]
    public void When_receiving_clearance_request_soap_Then_should_convert_to_json(string soapFileName, string rootName, SoapType soapType, string jsonFileName)
    {
        ...
        JsonToSoapConverter.Convert(json, knownArrays, rootName, soapType).LinuxLineEndings().Should().Be(soap);
    }
}