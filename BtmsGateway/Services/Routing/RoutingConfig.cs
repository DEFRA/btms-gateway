...
public record RoutingConfig
{
    public RoutedLink[] AllRoutes =>
        NamedRoutes.Join(NamedLinks, nr => nr.Value.LegacyLinkName, nl => nl.Key, (nr, nl) => new { Name = nr.Key, nr.Value.BtmsLinkName, LegacyLink = nl.Value.Link, LegacyLinkType = nl.Value.LinkType, nr.Value.SendRoutedResponseToFork, nr.Value.RouteTo })
                   .Join(NamedLinks, nrl => nrl.BtmsLinkName, nl => nl.Key, (nrl, nl) => new RoutedLink { Name = nrl.Name, LegacyLink = nrl.LegacyLink, LegacyLinkType = nrl.LegacyLinkType, BtmsLink = nl.Value.Link, BtmsLinkType = nl.Value.LinkType, SendLegacyResponseToBtms = nrl.SendRoutedResponseToFork, RouteTo = nrl.RouteTo })
                   .ToArray();
    
    public required List<KeyValuePair<string, NamedRoute>> NamedRoutes { get; init; } = [];
    public required List<KeyValuePair<string, NamedLink>> NamedLinks { get; init; } = [];
}
